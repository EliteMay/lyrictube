...
v0.10.1から **クラウド保存Writerは `cloud-sync.js` の1本だけ**です。以前の `site-shell.js` 全体保存処理は無効化しています。
...
`app.js` は旧バージョンから機能を積み重ねてきたため、まだ大きな単一ファイルです。v0.10.1では重複ランタイムとデータ・同期の土台を先に整理しました。今後は動作を維持したままPlayer / Lyrics / Library / UI単位に段階分割します。
