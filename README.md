...
タグ管理画面・サイドバー・曲一覧からの編集に加えて、Build `20260906-1` からは「曲を追加 / 曲情報を編集」ダイアログ内でも既存タグを選択できます。タグが無い場合は、その場で新しいタグを作成して選択できます。保存形式は既存の `song.tagIds` / `settings.tags` をそのまま利用し、Data Schema 4は変更しません。
